...
void drawEnemy(BUTTERFLY* b, int index) {
 if (b->active) {
  shadowOAM[index].attr0 = (0xFF & b->row) | (0<<13) | (0<<14);
  shadowOAM[index].attr1 = (0x1FF & b->col) | (1<<14);
  shadowOAM[index].attr2 = ((0)<<12) | ((4)*32+(b->currFrame*((b->width/8))));
 } else {
...
void drawFlower(FLOWER* f, int index) {
 if (f->active) {
  shadowOAM[index].attr0 = f->row | (0<<13) | (0<<14);
  shadowOAM[index].attr1 = f->col | (1<<14);
  shadowOAM[index].attr2 = ((0)<<12) | ((2)*32+(0));
 } else {
  shadowOAM[index].attr0 = (2<<8);
 }
}
